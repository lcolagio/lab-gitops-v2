...
# gturl="https://gitea-scm.apps.cluster-7jbph.7jbph.sandbox2892.opentlc.com"

#
## Get Token
export TOKEN=$(curl -s -X POST ${gturl}/api/v1/users/${gtadmin}/tokens -u ${gtadmin}:${gtpass} -H "Content-Type: application/json" -d '{"name": "admintoken2"}' | jq -r .sha1)
